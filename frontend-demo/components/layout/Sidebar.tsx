...
import { LayoutDashboard, TrendingUp, Server, MessageSquare, Zap } from 'lucide-react'
    ...
    <aside className="fixed left-0 top-0 h-full w-56 bg-card shadow-sidebar flex flex-col z-40">
      ...
      <div className="flex h-14 items-center gap-3 px-5 border-b border-border">
        ...
        <div className="min-w-0">
          <p className="text-sm font-semibold tracking-tight text-foreground leading-none">FiscalForge</p>
          <p className="text-[10px] text-muted-foreground mt-0.5 leading-none">FinOps Intelligence</p>
        </div>
      </div>

      {/* Section label */}
      <div className="px-5 pt-5 pb-1.5">
        <p className="text-[9px] font-semibold uppercase tracking-widest text-muted-foreground/70">Platform</p>
      </div>
      ...
      {/* Demo status */}
      <div className="mx-3 mb-3 rounded-md border border-primary/20 bg-primary/5 px-3 py-2.5">
        <div className="flex items-center gap-2">
          <span className="relative flex h-2 w-2">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-success opacity-60" />
            <span className="relative inline-flex rounded-full h-2 w-2 bg-success" />
          </span>
          <p className="text-[11px] font-semibold text-primary">Product Demo</p>
        </div>
        <p className="text-[10px] text-muted-foreground mt-0.5 pl-4">Simulated AWS data</p>
      </div>
    </aside>
  )
}
