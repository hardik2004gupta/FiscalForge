    ...
    <div className="rounded-lg border border-border bg-card p-5 shadow-card">
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-sm font-medium text-foreground">Daily AWS cost over time</p>
          {data.length > 0 && (
            <p className="text-xs text-muted-foreground mt-0.5">
              Avg {formatCurrency(avg)}/day · Peak {formatCurrency(maxCost)}
            </p>
          )}
        </div>
      ...
      <div className="h-60">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 2, right: 2, bottom: 0, left: 0 }}>
            <defs>
              <linearGradient id="costPageGradient" x1="0" y1="0" x2="0" y2="1">
